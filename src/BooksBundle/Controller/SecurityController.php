...
use BooksBundle\Entity\Restorer;
use BooksBundle\Form\UserType;
...
class SecurityController extends Controller
{
    ...
    public function resetPasswordAction(Request $request)
	{
	    ...
	    $restorer = $em->getRepository('BooksBundle:Restorer')->findOneByAuth(md5($token));
    ...
    public function lostPasswordAction(Request $request)
	{
	    ...
	    if($reset)
	    {
	    	$user = $em->getRepository('BooksBundle:User')->findOneByEmail($request->request->get('email'));
	    	if(!is_null($user))
	    	{
	    		$rb = uniqid(rand(), true);
	    		...
	    		$restorer = $em->getRepository('BooksBundle:Restorer')->findOneByUser($user);
	    		if(is_null($restorer))
	    		{
